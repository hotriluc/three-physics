...
const createSphere = (radius, position) => {
  ...
  ownMaterial.color.setHex(Math.random() * "0xffffff");
  const mesh = new THREE.Mesh(geometry, ownMaterial);
  ...
  return { threeObject: mesh, physicObject: sphereBody };
};

const createBoxes = (height, width, depth, position) => {
  const ownMaterial = material.clone();
  ownMaterial.color.setHex(Math.random() * "0xffffff");
  const mesh = new THREE.Mesh(boxGeometry, ownMaterial);
...
const tick = () => {
  ...
  objects.forEach((object) => {
    object.threeObject.position.copy(object.physicObject.position);
    object.threeObject.quaternion.copy(object.physicObject.quaternion);
  });
